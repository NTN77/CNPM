...
public class RateDAO {
    public static List<RateRequest> getAll() {
        try {
            return JDBIConnector.me().withHandle(handle ->
                    handle.createQuery("SELECT p.productId , product.name ,p.createDate,p.comment, COUNT(p.productId) AS TongDanhGia,AVG(p.starRatings) AS DanhGiaTrungBinh  " +
    ...
    public static List<RateDetailRequest> getRateDetailById(int id) {
        try {
            return JDBIConnector.me().withHandle(handle ->
                    handle.createQuery("SELECT p.productId, product.name AS productName, user.name AS userName, p.createDate, p.comment " +
                                    "FROM rate p " +
